...
export const drlReadProtocol = async (did: string, protocolIdUri: string, subpath: string) => {
  const dwebUrl = drlReadProtocolUrl(did, protocolIdUri, subpath)
  return fetch(dwebUrl)
}

export const drlReadProtocolJson = async (did: string, protocolIdUri: string, subpath: string) => {
